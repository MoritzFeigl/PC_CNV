...
# the patients with gleason 10 which have no gelson first/second data
relevant_seg[c(grep("797", relevant_seg$Proband),
               grep("811", relevant_seg$Proband),
               grep("863", relevant_seg$Proband),
               grep("901", relevant_seg$Proband),
               grep("1210", relevant_seg$Proband),
               grep("3948", relevant_seg$Proband),
               grep("10280", relevant_seg$Proband),
               grep("10605", relevant_seg$Proband),
               grep("10722", relevant_seg$Proband),
               grep("10875", relevant_seg$Proband),
               grep("11067", relevant_seg$Proband)), "cc"] <- "aggressive"
...
p1 <- ggplot(gains_ch, aes(x = factor(chr_n), y = status, fill = Group)) + 
  ...
  ggtitle("CN Gain") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.6, size = 12, margin=margin(5,0,10,0)),
...
p3 <- ggplot(cna_ch, aes(x = factor(chr_n), y = status, fill = Group)) + 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.6, size = 12, margin=margin(5,0,10,0)),
        ...
        axis.title = element_text(size = 10),
        axis.ticks.length = unit(8, "pt"),
        legend.text = element_text(size=16, margin = margin(t = 10, b = 10, l = 10, unit = "pt")),
...
p1 <- ggplot(gain_data, aes(x = Group, y = status, fill = Group)) + 
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5, size = 12, margin=margin(5,0,10,0)),
        axis.text.y = element_text(size = 12, margin=margin(0,5,0,10)),
        axis.title = element_text(size = 10),
        axis.ticks.length = unit(8, "pt"),
        legend.text = element_text(size=12),
...
# 3. model: CNV sum and age as covariate ---------------------------------------
if(model_group %in% c("loss", "cna")){
  model_df_age_small <- type_agg_age[!(type_agg_age$Proband %in% c("M00512_04_965_D11")), -c(1, 2, 3, 5:26)]
} else {
  model_df_age_small <- type_agg_age[, -c(1, 2, 3, 5:26)] # gains
}
model_df_age_small$aggressive <- factor(model_df_age_small$aggressive)
# Model
log_model3 <- glm(aggressive ~ .,
                  family = binomial, 
                  data = model_df_age_small)
summary(log_model3)
plot(log_model3)
# ROC curve
ROC_curve(log_model3, model_df_age_small, model_df_age_small$aggressive)

# 4. model: CNV subset and age as covariate ---------------------------------------
# Model

## Losses
if(model_group %in% c("loss", "cna")){
  model_df_age2 <- type_agg_age[!(type_agg_age$Proband %in% c("M00512_04_965_D11")), -c(1, 2, 3)]
} else {
  model_df_age2$aggressive <- factor(model_df_age2$aggressive)
}
log_model4 <- glm(aggressive ~ Chr8 + Chr9 + Chr10 +Chr_sum + Alter,
                  family = binomial, 
                  data = model_df_age2)

## Gains
model_df_age2 <- type_agg_age[, -c(1, 2, 3)]
model_df_age2$aggressive <- factor(model_df_age2$aggressive)
log_model4 <- glm(aggressive ~ Chr12 + Chr_sum + Alter,
                  family = binomial, 
                  data = model_df_age2)
...
# ROC curve
ROC_curve(log_model4, model_df_age2, model_df_age2$aggressive)
...
model_df_age2 <- type_agg_age[!(type_agg_age$Proband %in% c("M00512_04_965_D11")), -c(1, 2, 3)]
model_df_age2$aggressive <- factor(model_df_age2$aggressive)
log_model5 <- glm(aggressive ~ Chr8 + Chr9 + Chr10,
                  family = binomial, 
                  data = model_df_age2)
...
model_df_age2 <- type_agg_age[, -c(1, 2, 3)]
model_df_age2$aggressive <- factor(model_df_age2$aggressive)
log_model5 <- glm(aggressive ~ Chr12,
                  family = binomial, 
                  data = model_df_age2)
...
# ROC curve
ROC_curve(log_model5, model_df_age2, model_df_age2$aggressive)
